...
declare namespace java {
  export namespace net{
    interface InetAddress{
      getHostAddress():void
      getByName(name:string):java.net.InetAddress;
    }
  ...
  export namespace honzamrazek {
    export namespace simplenetworking {
      ...
      export class TcpClient {
        constructor(
          listener: cz.honzamrazek.simplenetworking.TcpClientListener
        );
      ...
      export class TcpServerListener {
        constructor(
          listener: cz.honzamrazek.simplenetworking.TcpServerListener
        );
        onClient(client: java.net.InetAddress): void;
        onData(client: java.net.InetAddress, data: string): void;
        onError(
      ...
      export class TcpServer {
        ...
        getClient(
          client: java.net.InetAddress
        ): cz.honzamrazek.simplenetworking.TcpClient;
      }
    }
  }
}
...
export class UdpServer {
  ...
  constructor() {
    let self = this;
    let listener = new cz.honzamrazek.simplenetworking.UdpListener({
  ...
  public send(address: any, packet: string): number {
    let name: string;
    if (address && typeof address == "string") name = address;
...
export class TcpClient {
  ...
  constructor() {
    ...
    let listener = new cz.honzamrazek.simplenetworking.TcpClientListener({
      onData: (data) => {
        if (self.onData !== null) self.onData(data);
      },
      onError: (id, message) => {
        if (self.onError !== null) self.onError(id, message);
      },
...
//     private client: cz.honzamrazek.simplenetworking.UdpClient;
//     public onData: {(data: string): void;};
//     public onError: {(id: number, message: string): void;};
//     public onFinished: {(id: number): void;};
...
//     constructor() {
//         let self = this;
//         let listener = new cz.honzamrazek.simplenetworking.UdpClientListener({
//             onData: (data) => {
//                 if (self.onData !== null)
//                     self.onData(data);
//             },
...
//             }
//         });
//         this.client = new cz.honzamrazek.simplenetworking.UdpClient(listener);
//     }
...
export class TcpServer {
  ...
  constructor(maxClients: number) {
    let self = this;
    let listener = new cz.honzamrazek.simplenetworking.TcpServerListener({
      onClient: (client) => {
    ...
    this.server = new cz.honzamrazek.simplenetworking.TcpServer(
      maxClients,
      listener
    );
  }
  ...
  public send(client: Address4, data: string): number {
    return this.server.send(
      java.net.InetAddress.getByName(client.address),
      data
    );
  }
}
